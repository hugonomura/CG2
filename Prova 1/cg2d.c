...
int FillObject(bufferdevice * buf, int color) {
  ...
  for(m=0;m<buf->MaxY;m++) {
	...
	par = 0;
	for(j=i;j<n;j++) buf->buffer[m*buf->MaxX+j] = color;
	} 
      if ((buf->buffer[m*buf->MaxX+n]!=0)&&(buf->buffer[m*buf->MaxX+n+1]==0)&&(!par)) {
	par = 1;
	i = n;
...
void CopyBuffer2Device(bufferdevice * buf, bufferdevice * dev) {
  ...
  for(m=0;m<buf->MaxY;m++) {
    for(n=0;n<buf->MaxX;n++) {
      if (buf->buffer[m*buf->MaxX+n] != 0) dev->buffer[m*dev->MaxX+n] = buf->buffer[m*buf->MaxX+n];
      }
    }  
...
point3d * ProjPerspectiva(point3d *p){
...
/*
int DumpAllegro(bufferdevice * dev, palette * pal) {
  int                   n, m, width, height, ret = 1;
  ColorValues           * cor;
  BITMAP                * img; 
  ...
  if (set_gfx_mode(GFX_AUTODETECT_WINDOWED,width,height,0,0) < 0) ret = 0;
  else {
    img = create_bitmap(width,height);
    for(m=0;m<height;m++) {
      for(n=0;n<width;n++) {
          cor = GetColor(dev->buffer[m*width+n],pal);
          putpixel(img,n,m,makecol(round((cor->red)*255),round((cor->green)*255),round((cor->blue)*255)));
          }
        }
...
void Desenha(void) {
  ...
  glBegin(GL_POINTS);
    for(m=0;m<devGL->MaxY;m++) {
      for(n=0;n<devGL->MaxX;n++) {
          cor = GetColor(devGL->buffer[m*(devGL->MaxX)+n],palGL);
          glColor3f(cor->red,cor->green,cor->blue);
          glVertex2i(n,devGL->MaxY-m);
          }
        }
  glEnd();
...
void AlteraTamanhoJanela(GLsizei w, GLsizei h) {
    ...
    glLoadIdentity();
    if (w <= h) gluOrtho2D(0.0f, devGL->MaxX, 0.0f, (devGL->MaxX)*h/w);
    else gluOrtho2D(0.0f, (devGL->MaxY)*w/h, 0.0f, devGL->MaxY);
    }    
*/
/*
int DumpOpenGL(int argc, char ** argv, bufferdevice * dev, palette * pal) {  
  ...
  glutInitWindowSize(dev->MaxX,dev->MaxY);
