//Exercicio 3 da Lista 5
//Proje��o em Perspectiva
...
void imagem( Ponto* iP, Ponto* P, Ponto* C, Ponto* W, Vetor* normal){
	float lambda=0;

	//Calculo do lambda
	lambda = (-(normal->a*P->x + normal->b*P->y + normal->c*P->z) - (W->x + W->y + W->z)) / ((normal->a*P->x - normal->a*C->x) + (normal->b*P->y - normal->b*C->y) + (normal->c*P->z - normal->c*C->z));
...
int main(){
	...
	//C = W + beta*(a,b,c);
	C.x = W.x + beta*normal.a;
	C.y = W.y + beta*normal.b;
	C.z = W.z + beta*normal.c;
	C.W = 1;

	//(x, y, z) = (c1, c2, c3) + L (x1 - c1, y1 - c2, z1 - c3)
	imagem(&iP1, &P1, &C, &W, &normal );
	imagem(&iP2, &P2, &C, &W, &normal );
	imagem(&iP3, &P3, &C, &W, &normal );
	imagem(&iP4, &P4, &C, &W, &normal );
	imagem(&iP5, &P5, &C, &W, &normal );
	imagem(&iP6, &P6, &C, &W, &normal );
	imagem(&iP7, &P7, &C, &W, &normal );
	imagem(&iP8, &P8, &C, &W, &normal );
